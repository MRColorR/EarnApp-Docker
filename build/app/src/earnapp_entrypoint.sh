...
if [ ! -x /usr/bin/earnapp ]; then
  ...
  # this commented parts freezes if ran withing the service but works if run standalone after in bash. Comment it out to try. 
...
# # 4) Start EarnApp
# echo "[earnapp_entrypoint.sh] Starting EarnApp..."
# # earnapp start
# earnapp status

echo "[earnapp_entrypoint.sh] EarnApp installed and started."
